 ...
 * WidgetColorSelector 设计，但完全基于本项目已使用的原版 Fabric 1.20.1 API
...
public class ColorPickerScreen extends Screen {
    ...
    protected void init() {
        ...
        ButtonWidget confirmButton = ButtonWidget.builder(Text.literal("确定"), button -> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        markerX = MathHelper.clamp(markerX, squareX, squareX + SQUARE_SIZE - 1);
        markerY = MathHelper.clamp(markerY, squareY, squareY + SQUARE_SIZE - 1);
        ...
        hueMarkerY = MathHelper.clamp(hueMarkerY, hueStripY, hueStripY + SQUARE_SIZE - 1);
    ...
    private void updateSaturationBrightness(double mouseX, double mouseY) {
        double relX = MathHelper.clamp(mouseX - squareX, 0.0, SQUARE_SIZE);
        double relY = MathHelper.clamp(mouseY - squareY, 0.0, SQUARE_SIZE);
    ...
    private void updateHue(double mouseY) {
        double relY = MathHelper.clamp(mouseY - hueStripY, 0.0, SQUARE_SIZE);
