...
public class SignGuideConfirmation1EntityRenderer extends AbstractTextDisplayEntityRenderer<SignGuideConfirmation1Entity> {
    ...
    @Override
    protected void applyTransforms(MatrixStack matrices, SignGuideConfirmation1Entity entity) {
    ...
    protected float getZOffset(SignGuideConfirmation1Entity entity) {
        return switch (SignTypeConverter.convert(entity.getCachedState().get(SignGuideConfirmation1.TYPE))) {
            case POLE_L -> -0.74f;
            case POLE_H -> -0.81f;
            case NORMAL -> -0.45f;
        };
