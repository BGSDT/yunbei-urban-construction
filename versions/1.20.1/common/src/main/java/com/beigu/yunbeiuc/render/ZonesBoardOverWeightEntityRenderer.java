package com.beigu.yunbeiuc.render;

import com.beigu.yunbeiuc.block.custom.sign.ZonesBoardOverWeight;
import com.beigu.yunbeiuc.entity.ZonesBoardOverWeightEntity;
import net.minecraft.client.render.block.entity.BlockEntityRendererFactory;
import net.minecraft.client.util.math.MatrixStack;
import net.minecraft.util.math.Direction;
import net.minecraft.util.math.RotationAxis;

public class ZonesBoardOverWeightEntityRenderer extends AbstractTextDisplayEntityRenderer<ZonesBoardOverWeightEntity> {

    public ZonesBoardOverWeightEntityRenderer(BlockEntityRendererFactory.Context ctx) {
        super(ctx);
    }

    @Override
    protected void applyTransforms(MatrixStack matrices, ZonesBoardOverWeightEntity entity) {
        Direction facing = entity.getCachedState().get(ZonesBoardOverWeight.FACING);
        matrices.translate(0.5, 0.5, 0.5);
        matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(-facing.asRotation()));
    }

    @Override
    protected float getZOffset(ZonesBoardOverWeightEntity entity) {
        return switch (SignTypeConverter.convert(entity.getCachedState().get(ZonesBoardOverWeight.TYPE))) {
            case POLE_L -> -0.74f;
            case POLE_H -> -0.81f;
            case NORMAL -> -0.45f;
        };
    }
}
