...
public class SignGuideLaneIndicator1EntityRenderer extends AbstractTextDisplayEntityRenderer<SignGuideLaneIndicator1Entity> {
    ...
    @Override
    protected void applyTransforms(MatrixStack matrices, SignGuideLaneIndicator1Entity entity) {
    ...
    protected float getZOffset(SignGuideLaneIndicator1Entity entity) {
        return switch (SignTypeConverter.convert(entity.getCachedState().get(SignGuideLaneIndicator1.TYPE))) {
            case POLE_L -> -0.74f;
            case POLE_H -> -0.81f;
            case NORMAL -> -0.45f;
        };
