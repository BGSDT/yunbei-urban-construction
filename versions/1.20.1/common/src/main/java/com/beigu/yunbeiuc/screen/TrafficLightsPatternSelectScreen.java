...
public class TrafficLightsPatternSelectScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addDrawableChild(
                ButtonWidget.builder(Text.literal("应用到该组"), button -> applyPreset())
                        .dimensions(panelX + 20, panelY + 50, buttonWidth, 20)
                        .build()
        );
        this.editButton = ButtonWidget.builder(Text.literal("编辑预设"), button -> openPatternEditor(true))
        ...
        this.deleteButton = ButtonWidget.builder(Text.literal("删除预设"), button -> deletePreset())
        ...
        this.newButton = ButtonWidget.builder(Text.literal("新建预设"), button -> openPatternEditor(false))
    ...
    private void openPatternEditor(boolean editSelected) {
        if (editSelected) {
            ...
            MinecraftClient.getInstance().setScreen(new TrafficLightsPatternEditorScreen(preset, this.pos, this.category, this));
            return;
        }
        MinecraftClient.getInstance().setScreen(new TrafficLightsPatternEditorScreen(this.pos, this.category, this));
    }
    ...
    private class NameListWidget extends AbstractOptionListWidget<String> {
        public NameListWidget(MinecraftClient client, int width, int height, int top, int bottom, int itemHeight,
                               List<String> names) {
