...
public class SignGuideIntersectionWarning4EntityRenderer extends AbstractTextDisplayEntityRenderer<SignGuideIntersectionWarning4Entity> {
    ...
    @Override
    protected void applyTransforms(MatrixStack matrices, SignGuideIntersectionWarning4Entity entity) {
    ...
    protected float getZOffset(SignGuideIntersectionWarning4Entity entity) {
        return switch (SignTypeConverter.convert(entity.getCachedState().get(SignGuideIntersectionWarning4.TYPE))) {
            case POLE_L -> -0.74f;
            case POLE_H -> -0.81f;
            case NORMAL -> -0.45f;
        };
