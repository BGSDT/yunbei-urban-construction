...
// 缩放按钮
    ...
    private class ImageListWidget extends AbstractOptionListWidget<ImageOption> {
        public ImageListWidget(MinecraftClient client, int width, int height, int top, int bottom, int itemHeight,
                                List<ImageOption> imageOptions, Consumer<ImageOption> onSelect) {
            super(client, width, height, top, bottom, itemHeight, imageOptions,
                    option -> option == selectedOption, onSelect,
                    option -> Text.translatable(option.getTranslationKey()), ImageOption::getColor);
        }
    }
