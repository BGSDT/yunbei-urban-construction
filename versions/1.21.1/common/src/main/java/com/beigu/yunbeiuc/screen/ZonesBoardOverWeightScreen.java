...
import com.beigu.yunbeiuc.network.ZonesBoardOverWeightUpdatePacket;
...
public class ZonesBoardOverWeightScreen extends Screen {
    ...
    public ZonesBoardOverWeightScreen(BlockPos pos) {
        super(Text.translatable("text.yunbeiuc.zones_board_over_weight.title"));
        this.pos = pos;
    }
    ...
    protected void init() {
        ...
        if (this.client != null && this.client.world != null) {
            if (this.client.world.getBlockEntity(this.pos) instanceof ZonesBoardOverWeightEntity entity) {
        ...
        this.addDrawableChild(
                ButtonWidget.builder(Text.translatable("text.yunbeiuc.zones_board_over_weight.save"), button -> this.saveAndClose())
                        .dimensions(panelX + 60, buttonY, 90, 24)
                        .build()
        );

        this.addDrawableChild(
                ButtonWidget.builder(Text.translatable("text.yunbeiuc.zones_board_over_weight.cancel"), button -> this.close())
    ...
    private TextFieldWidget createTextField(int x, int y, int width, String existingText) {
        TextFieldWidget field = new TextFieldWidget(
                ...
                x, y,
                width, INPUT_HEIGHT,
                Text.translatable("text.yunbeiuc.zones_board_over_weight.content")
        );
        field.setMaxLength(256);
        field.setText(existingText);
        field.setPlaceholder(Text.translatable("text.yunbeiuc.zones_board_over_weight.placeholder"));
    ...
    private void saveAndClose() {
        if (this.client != null && this.client.world != null) {
            ...
            ZonesBoardOverWeightUpdatePacket packet = new ZonesBoardOverWeightUpdatePacket(pos, text1);
            RegistryByteBuf buf = new RegistryByteBuf(Unpooled.buffer(), this.client.getNetworkHandler().getRegistryManager());
            packet.write(buf);
            NetworkManager.sendToServer(ModMessages.UPDATE_ZONES_BOARD_OVER_WEIGHT, buf);
        }
        this.close();
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.zones_board_over_weight.title"),
        ...
        context.drawTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.zones_board_over_weight.text_1_name"),
                panelX + 10, panelY + 16,
                0xFFAAAAAA
        );
