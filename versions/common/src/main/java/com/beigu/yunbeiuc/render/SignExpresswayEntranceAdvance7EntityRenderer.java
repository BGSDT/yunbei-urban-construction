...
public class SignExpresswayEntranceAdvance7EntityRenderer extends AbstractTextDisplayEntityRenderer<SignExpresswayEntranceAdvance7Entity> {
    ...
    @Override
    protected void applyTransforms(PoseStack matrices, SignExpresswayEntranceAdvance7Entity entity) {
        Direction facing = entity.getBlockState().getValue(SignExpresswayEntranceAdvance7.FACING);
    ...
    protected float getZOffset(SignExpresswayEntranceAdvance7Entity entity) {
        return switch (SignTypeConverter.convert(entity.getBlockState().getValue(SignExpresswayEntranceAdvance7.TYPE))) {
            case POLE_L -> -0.74f;
            case POLE_H -> -0.81f;
            case NORMAL -> -0.45f;
        };
