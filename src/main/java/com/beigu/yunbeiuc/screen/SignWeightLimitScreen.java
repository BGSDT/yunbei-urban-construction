...
import com.beigu.yunbeiuc.network.abandoned.UpdateWeightLimitPacket;
...
public class SignWeightLimitScreen extends Screen {
    private final List<WeightLimitOption> options;
    private WeightLimitListWidget listWidget;
    private SignWeightLimit selectedLimit;
    private final BlockPos blockPos;

    public SignWeightLimitScreen(Text title, BlockPos pos, SignWeightLimit currentLimit) {
        super(title);
        this.blockPos = pos;
        this.options = createWeightLimitOptions();

        // 确保使用传入的当前重量限制
        this.selectedLimit = currentLimit;
        System.out.println("GUI创建：当前重量限制: " + this.selectedLimit.name());
    }
    ...
    protected void init() {
        ...
        if (this.client != null && this.client.world != null) {
            ...
            if (blockEntity instanceof SignWeightLimitBlockEntity weightLimitBlockEntity) {
                this.selectedLimit = weightLimitBlockEntity.getWeightLimit();
                System.out.println("GUI初始化：读取当前重量限制: " + this.selectedLimit.name());
            }
        }

        // 列表宽度调整为屏幕的1/2
        int listWidth = this.width / 2;
        this.listWidget = new WeightLimitListWidget(
                this.client,
                listWidth,
                this.height,
                40,
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.sign_weight_limit.title"),
        ...
        context.drawTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.sign_weight_limit.current_selection", getWeightValue(selectedLimit)),
                listAreaLeft + 10,
                this.height - 55,
                0xFFFFFF
        );
    ...
    private void drawInfoPanel(DrawContext context) {
        ...
        context.drawCenteredTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.sign_weight_limit.info_panel"),
                panelX + panelWidth / 2,
                panelY + 10,
                0xFFFFFF
        );
        ...
        // 绘制小图标
        drawWeightLimitImage(context, iconX, iconY, iconSize);
        ...
        context.drawTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.sign_weight_limit.weight", getWeightValue(selectedLimit)),
                infoX,
                infoY,
                0xFFFFFF
        );
        ...
        context.drawCenteredTextWithShadow(
                this.textRenderer,
                Text.translatable("text.yunbeiuc.sign_weight_limit.save_button"),
    ...
    private void drawInfoBox(DrawContext context, int topY) {
        ...
        // 绘制长文本内容（自动换行）
        String description = Text.translatable("text.yunbeiuc.sign_weight_limit.description").getString();
    ...
    private void drawWeightLimitImage(DrawContext context, int x, int y, int displaySize) {
        // 根据当前选择的重量构建图片路径
        String imagePath = getWeightImagePath(selectedLimit);
        ...
        try {
            ...
            context.drawCenteredTextWithShadow(
                    this.textRenderer,
                    Text.translatable("text.yunbeiuc.sign_weight_limit.load_failed"),
                    x + displaySize/2,
                    y + displaySize/2 - 5,
                    0xFFFFFF
            );
    ...
    private String getWeightImagePath(SignWeightLimit limit) {
        // 根据枚举值返回对应的图片路径
        String weightNumber = limit.asString().replace("sign_weight_limit_", "").replace("sign_alex_weight_limit_", "");
        return "textures/gui/sign_weight_limit_" + weightNumber + ".png";
    }
    ...
    private void saveSelection() {
        System.out.println("保存重量限制: " + selectedLimit.asString() + " (" + getWeightValue(selectedLimit) + " t)");
        System.out.println("方块位置: " + blockPos);

        // 发送网络数据包到服务器
        if (client != null && client.getNetworkHandler() != null) {
            PacketByteBuf buf = PacketByteBufs.create();
            new UpdateWeightLimitPacket(blockPos, selectedLimit).write(buf);
            ClientPlayNetworking.send(ModMessages.UPDATE_WEIGHT_LIMIT, buf);
        }
        ...
        if (client.world != null) {
            ...
            if (blockEntity instanceof SignWeightLimitBlockEntity) {
                ((SignWeightLimitBlockEntity) blockEntity).setWeightLimit(selectedLimit);
            }
        }
    }

    private List<WeightLimitOption> createWeightLimitOptions() {
        List<WeightLimitOption> options = new ArrayList<>();
        for (SignWeightLimit limit : SignWeightLimit.values()) {
            options.add(new WeightLimitOption(limit, getWeightValue(limit)));
        }
        return options;
    }

    private String getWeightValue(SignWeightLimit limit) {
        String numberStr = limit.asString().replace("sign_weight_limit_", "").replace("sign_alex_weight_limit_", "");
        return String.valueOf(Integer.parseInt(numberStr));
    }

    public void setSelectedLimit(SignWeightLimit limit) {
        this.selectedLimit = limit;
    }

    private static class WeightLimitOption {
        private final SignWeightLimit limit;
        private final String displayText;

        public WeightLimitOption(SignWeightLimit limit, String displayText) {
            this.limit = limit;
            this.displayText = displayText + " t";
        }

        public SignWeightLimit getLimit() {
            return limit;
        }
    ...
    private class WeightLimitListWidget extends ElementListWidget<WeightLimitListWidget.Entry> {
        private final List<WeightLimitOption> options;
        private final int listWidth;

        public WeightLimitListWidget(MinecraftClient client, int width, int height, int top, int bottom, int itemHeight) {
            super(client, width, height, top, bottom, itemHeight);
            this.listWidth = width;
            this.options = createWeightLimitOptions();

            for (WeightLimitOption option : options) {
        ...
        public class Entry extends ElementListWidget.Entry<Entry> {
            private final WeightLimitOption option;

            public Entry(WeightLimitOption option) {
                this.option = option;
            }
            ...
            public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
                ...
                try {
                    // 获取对应重量的图片路径
                    String imagePath = getWeightImagePath(option.getLimit());
                    Identifier texture = new Identifier("yunbeiuc", imagePath);
