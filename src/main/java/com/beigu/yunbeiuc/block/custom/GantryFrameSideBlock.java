...
import com.beigu.yunbeiuc.block.custom.data.GantryFrameType;
...
public class GantryFrameSideBlock extends Block {
    public static final EnumProperty<GantryFrameType> FRAME_TYPE = EnumProperty.of("frame_type", GantryFrameType.class);

    public GantryFrameSideBlock(Settings settings) {
        super(settings);
        this.setDefaultState(this.getStateManager().getDefaultState().with(FRAME_TYPE, GantryFrameType.GANTRY_FRAME_SIDE_1));
    }

    @Override
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(FRAME_TYPE);
    }
    ...
    private BlockPos findBottomFrame(WorldAccess world, BlockPos startPos) {
        ...
        while (world.getBlockState(belowPos).getBlock() instanceof GantryFrameSideBlock) {
            currentPos = belowPos;
    ...
    private int getPositionFromBottom(WorldAccess world, BlockPos bottomPos, BlockPos targetPos) {
        ...
        while (!currentPos.equals(targetPos)) {
            ...
            if (!(world.getBlockState(currentPos).getBlock() instanceof GantryFrameSideBlock)) {
    ...
    private void updateConnectedFrames(World world, BlockPos pos) {
        ...
        if (currentState.getBlock() instanceof GantryFrameSideBlock) {
        ...
        if (aboveState.getBlock() instanceof GantryFrameSideBlock) {
