                        ...
                        .then(CommandManager.literal("lights")
                                .then(CommandManager.argument("mode", IntegerArgumentType.integer(1, 2))
                                        .executes(context -> {
                                            ...
                                            if (player == null) {
                                                source.sendError(new LiteralText("该命令只能由玩家执行"));
                                                return 0;
                                            }
                                            ...
                                            if (mode == 1) {
                                                ...
                                                player.sendMessage(new LiteralText("✓ 已选择十字路口模式，现在可以使用链接法杖点击红绿灯了！"), false);
                                                player.sendMessage(new LiteralText("需要选择8个红绿灯：每个方向各一个左转和一个直行"), false);
                                                return 1;
                                            } else if (mode == 2) {
                                                LinkWand.setPlayerMode(player, 2);
                                                player.sendMessage(new LiteralText("✓ 已选择T字路口模式，现在可以使用链接法杖点击红绿灯了！"), false);
                                                player.sendMessage(new LiteralText("需要选择3个红绿灯：三个不同方向各一个直行红绿灯（北东南/北东西/北西南/东西南）"), false);
                                                return 1;
                                            }
                                ...
                                .then(CommandManager.argument("action", StringArgumentType.word())
                                        ...
                                        .executes(context -> {
                                            ...
                                            if (!action.equalsIgnoreCase("confirm") &&
                                                ...
                                                player.sendMessage(new LiteralText("无效的操作！可用操作: confirm, reset, cancel"), false);
                                                return 0;
                                            }
